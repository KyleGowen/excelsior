...
- The deck section shows total, legal, and Limited deck counts; all-deck and legal-only averages per standard user that exclude signup starter-deck copies; users with positive collection quantities; and average summed card quantities across those active collectors only.
- The new-standard-accounts KPI and acquisition summary show the calculated start date of a rolling 30-day window relative to the snapshot time. Acquisition bars use intelligence blue for earlier months and cyan for months overlapping that rolling window. The y-axis scales to the observed maximum rounded to five, keeping low-volume months readable.
