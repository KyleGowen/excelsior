...
- **Venture value:** Reads catalog numerical fields rather than image text: `value` for Power/Special/Aspect cards, `acts_as` for Teamwork, `attack_value` for Ally Universe, and `bonus` for Basic/Training Universe. When an older Special row is missing `value`, the analyzer falls back to its printed `Acts as a level N` effect; explicit `does not count to/toward Venture Total` text still contributes zero. Nonnumerical cards contribute zero. Values are summed only for the copies the hand may keep after duplicate rules.
