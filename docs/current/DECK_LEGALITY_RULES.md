...
| Pre-placed Basic Universe (Dracula's Armory / The Sanctuary) | Up to **3 unique** Basic Universe cards may be pre-placed under the enabling location | `pre_placed_basic_universe_limit` — at most 3 flagged (`exclude_from_draw`) `basic-universe` groups; `pre_placed_basic_universe_unique` — each pre-placed `cardId` at most once. A persisted group may have quantity > 1, but its boolean marks exactly one physical copy pre-placed. **Only enforced when Dracula's Armory or The Sanctuary is in the deck** | Not checked | Not in client mirror (v2 sends `exclude_from_draw`; enforced server-side) |
| Pre-placed Training (Spartan Training Ground / Teen Team Headquarters) | Up to **3 unique** Training cards may be pre-placed under the enabling location | `pre_placed_training_limit` — at most 3 flagged (`exclude_from_draw`) `training` groups; `pre_placed_training_unique` — each pre-placed `cardId` at most once. A persisted group may have quantity > 1, but its boolean marks exactly one physical copy pre-placed. **Only enforced when Spartan Training Ground or Teen Team Headquarters is in the deck** | Not checked | Not in client mirror (v2 sends `exclude_from_draw`; enforced server-side) |
| Power cards usable | Grid / type (~L65–L71) | `unusable_power` — DB `power_type` / `value`; **Energy / Combat / Brute Force / Intelligence** vs matching stat; **Any-Power** uses max of the four; **Michonne's Combat is 8 when Rick Grimes and Alexandria started on her team**; **Alpha may use level 6 and 7 Intelligence Power cards when Walkers: Herd started on her team**; **Multi Power** and **Multi-Power** are **not** gated by character stats (usable in any Venture deck) | Not checked | **Aligned:** client applies the same Michonne and Alpha starting-team permissions and skips the grid check for Multi Power / Multi-Power |
...
**Starting-team Power-card permissions:** Michonne's printed Combat 7 becomes an effective Combat 8 only when **Rick Grimes** and **Alexandria** are both in the starting deck (including Reserve). The effective grid is used by server legality, Add Cards usability, deck max-stat displays, and Simulate KO. **Alpha and the Whisperers** may use level 6 and 7 Intelligence Power cards when **Walkers: Herd** is in the starting deck (including Reserve); this is a narrow Power-card permission and does not raise Alpha's displayed Intelligence grid or satisfy other Intelligence/Any-Power requirements. Because both cards say "started the game," their conditions remain active if an enabling teammate is later simulated as KO'd.
...
| 2026-09-21 | **Alpha and the Whisperers:** level 6 and 7 Intelligence Power cards are legal when Walkers: Herd started on Alpha's team. Server legality, Add Cards usability, and Simulate KO dimming are aligned without changing Alpha's printed grid or other card requirements. |
