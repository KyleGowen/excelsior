...
- Keep Storybook in sync with v2 UI work. Existing stories import production components and CSS, so edits to those files appear in the gallery automatically. Add new global CSS imports to `frontend/src/styles/appStyles.ts`, shared by the site and Storybook.
- For every new React component in `frontend/src/`, add a named Storybook example under `frontend/src/stories/` in the same change. This includes shared, feature-local, route, layout, modal, and mobile components regardless of reusability. If a component only makes sense inside its parent or provider, give the parent a named story variant that exercises the new component; do not export private internals solely for Storybook. For a new route page or visual state, add or update its screen story with fictional fixtures and mocked API responses. Remove or update stories when their production UI is removed or changed.
- Before finishing frontend UI work, check every newly added React component against the stories to confirm each has a named example, then run `npm --prefix frontend run build:storybook` alongside the normal frontend checks. See [`frontend/STORYBOOK.md`](frontend/STORYBOOK.md). Storybook is a local development gallery and does not change the production site.
